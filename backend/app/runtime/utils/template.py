...
# @update 2026-08-22 _lookup 三分支收敛为模式表驱动
...
def _lookup(template: str, context: dict[str, Any]) -> Any:
    ...
    for pattern, root in _PATTERNS:
        ...
        if name is None:
            ...
            root_value = context.get(root, {}).get(name)
        if root_value is None:
            return False, None

        path = match.group("path")
        val = _get_path(root_value, path.split(".") if path else [])
